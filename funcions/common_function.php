...
//Condition To Check Isset or not
    if (isset($_GET['state'])) {
        $state_title = $_GET['state'];

        $select_query = "SELECT * FROM `products` WHERE state_title=$state_title";
        $result_query = mysqli_query($conn, $select_query);
        $num_rows = mysqli_num_rows($result_query);
        if ($num_rows == 0) {
...
// echo $row_data['state_title'];
    ...
    <a href='productpage.php?state=$state_title' class=' nav-link text-light'>
        <h4>$state_title</h4>
    </a>
</li>";
